// plotTreeField_skim.C: a macro adapted to work on a skim file
//  I did not succeed to plot functions like CalcProbeHcalEtot().
//
// plotTreeField.C
...
void plotTreeFields_skim(TString fname, TString treeName, TString outFname,
			 ...
			 int keepCanvasesOpen=0)
{
  ...
  */
  ...
  if (treeName=="gjet_data") {
    TBranch *mainBranch= (TBranch*)ttree->GetBranch(treeName);
    branchList= mainBranch->GetListOfBranches();
  }
  ...
  while ( (brEl = (TChainElement*)branchIter()) ) {
    ...
    if (treeName=="gjet_data"){
      int skip=0;
      if (brName=="fAInfo") {
	brName="fAInfo.fRunNo";
	skip=1;
      }
      else if (brName=="fTagHcalE") {
	brName="CalcTagHcalEtot()";
	skip=1;
      }
      if (brName=="fProbeHcalE") {
	brName="CalcProbeHcalEtot()";
	skip=1;
      }
      if (skip) {
	std::cout << "skipping the class branch\n";
	continue;
      }
    }
